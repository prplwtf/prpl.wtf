---
title: new beginnings
description: a brand new personal website
tags: one, two
image: hi.jpg
date: 2026-09-02
unlisted: false
---

hi! it's me, emma.

welcome to my brand new personal blog. it's like my old writea-based blog, but good and a lot less corporate.

i plan to write things down here every now and then. i like writing things, so i might as well give this a try :)

- my blogging wont have any schedule
- i write about whatever i feel like :)
- these posts will be free to access :D
- bwaaaaaaaaaaaaaaaaaa
- explodes
- nyaaa

| nyaa | profit |
| ---- | ------ |
| 1x   | 100%   |
| 2x   | 150%   |
| 3x   | -100%  |

## summary

long story short: too much nyaa is bad for business, but little nyaa is fine, :P

also bwaaaaaaaaaaaaaaaaaaaaa

:sob: what the fuck am i writing at this point

```md
oh heres a codeblock i gotta make it work right

# heres a markdown title in a codeblock wow
```

```js [test.js]
console.log('codeblock with a name :(')
```

### what did we learn

why would you want to start a blog when

1. you have to deal with a bunch of markdown
2. you have to deal with text
3. you have to deal with writing

`or even code oh no` it also has to be inline :c

> QUOTES?? OH NO OH GOD PLEASE STOP

---

NOOO THERES A LINE TOO

lets hope emma doesnt try to add custom emoji :fear:

_italic text wow_

gambling gambling gambling

explodes violently
